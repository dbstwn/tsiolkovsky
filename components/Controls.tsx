...
const Slider: React.FC<SliderProps> = ({ label, value, min, max, step, onChange, unit, colorClass = "bg-blue-500" }) => (
  <div className="mb-4">
...
export const Controls: React.FC<ControlsProps> = ({ 
    params, setParams, visMode, setVisMode, solverTime, isRunning, setIsRunning, handleReset 
}) => {
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-5">
          ...
          {activeTab === 'preset' && (
              <div className="space-y-6">
                 ...
                 {/* Visual Field Selector (Moved from View) */}
                 <div>
                    <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">Visualization Mode</div>
                    <div className="grid grid-cols-2 gap-2">
                        {Object.entries(VisualizationMode).map(([key, label]) => (
                            <button
                                ...
                                className={`
                                    text-[10px] font-bold py-2.5 px-3 rounded-md transition-all border
                                    ${visMode === label 
                 ...
                 {/* Quick Conditions */}
                 <div>
                    <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">Quick Adjust</div>
          ...
          {/* VIEW TAB (Visuals only) */}
          {activeTab === 'view' && (
              <div className="space-y-6">
                  {/* Vis Mode removed from here */}
                  
                  <div>
                     <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">Numerical Stability</div>
                     <Slider label="CFL Number" value={params.cfl} min={0.1} max={0.9} step={0.05} unit="" onChange={(v) => setParams(p => ({ ...p, cfl: v }))} colorClass="bg-emerald-600" />
                  </div>
              </div>
          )}
      </div>
...
const PresetBtn = ({ label, sub, icon, color, bg, onClick }: any) => (
    <button 
        onClick={onClick}
        className="flex items-center gap-3 p-3 rounded-lg bg-slate-800 hover:bg-slate-700 border border-white/5 hover:border-white/10 transition-all group w-full text-left"
    >
        <div className={`w-8 h-8 rounded-md ${bg} flex items-center justify-center ${color} font-bold text-sm group-hover:scale-110 transition-transform`}>
            {icon}
        </div>
        <div>
            <div className="text-xs font-bold text-slate-200 group-hover:text-white">{label}</div>
